...
module.exports = {
  create_report: function (
    ...
    doc.registerFont("PTSansRegular", config.nalaz_ptsansregular);
    doc.registerFont("PTSansBold", config.nalaz_ptsansbold);
    doc.image(config.nalaz_logo + code + ".jpg", logoX, logoY, {
      width: 280,
    ...
    if (EN == true) {
      ...
      doc
        .font("PTSansRegular", config.nalaz_ptsansbold)
        .fontSize(10)
        .text("Datum und Uhrzeit der Probenahme:", 390, nvisina + 18 + 30 + 3);
      doc
        .font("PTSansRegular", config.nalaz_ptsansbold)
        .fontSize(10)
        .text(
          ...
          if (multi.length) {
            multi.forEach((mul) => {
              ...
              if (
                ...
                mul.naslov.slice(4).trim().includes("Pregled sedimenta urina")
              ) {
    ...
    if (EN === true) {
      ...
      if (napomena.trim().length) {
        ...
        for (var i = 0, l = eachLineNapomena.length; i < l; i++) {
          if (eachLineNapomena[i].includes("Analysis: Antigen SARS-CoV-2")) {
            doc
              .font("PTSansBold")
