  ...
  parsaj_rezultat: function (record, io,serijski) {
    ...
    //******************************************************** */
                                        ...
                                        if (element.status != "ODOBREN") {
                                          ...
                                          console.log(':: Dosao test sa CL-900i: ' + elementu.labassay.naziv)
                  ...
                  AnaAssays.find({ aparat: mongoose.Types.ObjectId(serijski)}).populate('aparat test').lean().exec(function (err, anaassays) {
                    ...
                    Results.findOne({ 'id': uzorak.id }).populate('patient rezultati.labassay').exec(function (err, rezultat) {
                      ...
                      if (testovi.length < 1) {
                        ...
                        var order = 'O|1||' + json.sid + '|' + tests + '|R|'+ makedate(stamp)+'|'+ makedate(stamp)+'||||||||'+tipu+'|'+uzorak.created_by+'|Atrijum|1|lejla||||||Q|||||';
//O|1|1^1^1|SAMPLE123|1^Test1^2^1\2^Test2^2^1\3^Test3^2^1\4^Test4^2^1|R|20090910135300|20090910125300|||John|||||Urine|Dr.Who|Department1|1|Dr.Tom||||||Q|||||
                        var niztest = []                      
                          if (order.length > 240) {
                          niztest = tests.split("^^^")
                          niztest.splice(0, 1);
