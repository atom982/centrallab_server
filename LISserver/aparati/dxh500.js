          ...
          record.forEach(function(element) {
              ...
              switch (record_type) {
                          ...
                          case 'O':
                                    ...
                                    var order=header[2].split("!");
                                    //console.log(header)//'DFH!'
                                    sid = order[0].trim()
                                    sifra_p='KKS5'
                                    break;
                          case 'R':
                                    var result = element.split("|");
                                    // 'R|1|!!!WBC|0.06  !-  |x10e3/uL||0.2 to 100|A||||LAB||20210121110439|45',
                                    var datum = result[12].substring(0,9)
                                    var vrijeme = result[12].substring(9,9)
                                    vrijeme_rezultata=datum
                                     rezultati.push({
                                      analit:result[2].split("!")[3],
                                      analit_rez: result[3].includes("--") ? "" : result[3].split("!")[0].trim(),
                                      analit_status:''
                                    })        
